...
MAT4 LookAt(const vec3 pos, const vec3 point, const vec3 up)
{
	vec3 V = normalize(point - pos);
	vec3 A = normalize(cross(V,up));
	vec3 B = cross(A,V);
