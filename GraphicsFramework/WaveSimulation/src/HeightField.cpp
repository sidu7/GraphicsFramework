...
#else	
			float u_inext = i == grid.width() - 1 ? 0.0f : grid(i+1,j);
			float u_iprev = i == 0 ? 0.0f : grid(i-1,j);
			float u_jnext = j == grid.height() - 1 ? 0.0f : grid(i,j+1);
			float u_jprev = j == 0 ? 0.0f : grid(i,j-1);
#endif				
			float inside = (u_inext - 2 * grid(i,j) + u_iprev) / pow(grid.cellWidth(),2) + (u_jnext - 2 * grid(i,j) + u_jprev) / pow(grid.cellHeight(),2);
			float temp = grid(i,j);
