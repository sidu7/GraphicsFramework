...
std::vector<Mesh::TextureData> Model::LoadMaterialTextures(aiMaterial* material, aiTextureType type, std::string typeName, const aiScene* scene)
{
	...
	for (unsigned int i = 0; i < material->GetTextureCount(type); ++i)
	{
		...
		{
			if (std::strcmp(loadedTextures[i].texture->GetSourcePath().c_str(), str.C_Str()) == 0)
			{
		...
		if (!skip)
		{
			...
			{
				data.texture = RenderingFactory::Instance()->CreateTexture();
				data.texture->Init(tex->pcData, tex->mWidth);
				data.type = typeName;
			...
			else
			{
				...
				data.texture = RenderingFactory::Instance()->CreateTexture();
				data.texture->Init(directory + '/' + fileName);
				data.type = typeName;
				textures.push_back(data);
				loadedTextures.push_back(data);
			}
